...
func _on_cell_hovered(pos: Vector2i) -> void:
	...
	var cells := _ship_cells(pos, _orientation, length)
	_grid_ui.set_preview_cells(cells)
...
func _ship_cells(pos: Vector2i, facing: Vector2i, length: int) -> Array[Vector2i]:
...
func _refresh_grid_overlay() -> void:
	...
	for ship in _game_state.ships:
		...
		for cell in _ship_cells(ship["position"], ship["facing"], ship["length"]):
			cells[cell] = color
